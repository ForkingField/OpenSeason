...
#define MAKE_FONT_DOWNLOAD_URL(name) "https://www.openseason.org/runtime-resources/fonts/" name
...
void QtHost::UpdateApplicationLanguage(QWidget* dialog_parent)
{
  ...
  const QString path = QStringLiteral("%1/openseason-qt_%3.qm").arg(base_dir).arg(language);
...
static constexpr const QtHost::GlyphInfo s_glyph_info[] = {
...

  {
    ...
    // auto update by generate_update_glyph_ranges.py with openseason-qt_ja.ts
  ...
  },
  {
    ...
    // auto update by generate_update_glyph_ranges.py with openseason-qt_ko.ts
  ...
  },
  {
    ...
    // auto update by generate_update_glyph_ranges.py with openseason-qt_zh-cn.ts
