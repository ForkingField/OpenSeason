    ...
    case Page_BIOS:
    {
      if (!BIOS::HasAnyBIOSImages())
      {
        if (QMessageBox::question(
              this, tr("Warning"),
              tr("No BIOS images were found. OpenSeason WILL NOT be able to run games without a BIOS image.\n\nAre "
      ...
      if (m_ui.searchDirectoryList->rowCount() == 0)
      {
        if (QMessageBox::question(
              ...
              tr("No game directories have been selected. You will have to manually open any game dumps you "
                 "want to play, OpenSeason's list will be empty.\n\nAre you sure you want to continue?")) !=
            QMessageBox::Yes)
        {
          return false;
        }
...
void SetupWizardDialog::confirmCancel()
{
  if (QMessageBox::question(this, tr("Cancel Setup"),
                            tr("Are you sure you want to cancel OpenSeason setup?\n\nAny changes have been saved, and "
                               "the wizard will run again next time you start OpenSeason.")) != QMessageBox::Yes)
  {
    return;
  }
