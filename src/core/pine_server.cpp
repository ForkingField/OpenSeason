...
bool PINEServer::Initialize(u16 slot)
{
  ...
  if (slot != Settings::DEFAULT_PINE_SLOT)
    socket_path = fmt::format("{}/openseason.sock.{}", runtime_dir, slot);
  else
    socket_path = fmt::format("{}/openseason.sock", runtime_dir);
...
bool PINEServer::PINESocket::HandleCommand(IPCCommand command, BinarySpanReader rdbuf)
{
  ...
  switch (command)
  {
    ...
    case MsgVersion:
    {
      const TinyString version = TinyString::from_format("OpenSeason {}", g_scm_tag_str);
