...
void Achievements::DrawGameOverlays()
{
  ...
  if (s_active_progress_indicator.has_value())
  {
    ...
    const ImVec2 text_size = g_medium_font->CalcTextSizeA(g_medium_font->LegacySize, FLT_MAX, 0.0f, text_start, text_end);
    ...
    dl->AddText(g_medium_font, g_medium_font->LegacySize, text_pos, col, text_start, text_end, 0.0f, &text_clip_rect);
  ...
  if (!s_active_leaderboard_trackers.empty())
  {
    for (auto it = s_active_leaderboard_trackers.begin(); it != s_active_leaderboard_trackers.end();)
    {
      ...
      const ImVec2 size = ImGuiFullscreen::g_medium_font->CalcTextSizeA(
        ImGuiFullscreen::g_medium_font->LegacySize, FLT_MAX, 0.0f, width_string.c_str(), width_string.end_ptr());
      ...
      const ImVec2 text_size = ImGuiFullscreen::g_medium_font->CalcTextSizeA(
        ImGuiFullscreen::g_medium_font->LegacySize, FLT_MAX, 0.0f, indicator.text.c_str(),
      ...
      dl->AddText(g_medium_font, g_medium_font->LegacySize, text_pos, text_col, indicator.text.c_str(),
      ...
      dl->AddText(g_medium_font, g_medium_font->LegacySize, icon_pos, text_col, ICON_FA_STOPWATCH, nullptr, 0.0f,
                  &text_clip_rect);
...
void Achievements::DrawPauseMenuOverlays()
{
  ...
  const float start_y = LayoutScale(10.0f + 4.0f + 4.0f) + g_large_font->LegacySize + (g_medium_font->LegacySize * 2.0f);
  ...
  const float title_height = padding + font->LegacySize + padding;
  ...
  if (!s_active_challenge_indicators.empty())
  {
    ...
    dl->AddText(font, font->LegacySize, ImVec2(box_min.x + padding, box_min.y + padding), IM_COL32(255, 255, 255, 255),
                TRANSLATE("Achievements", "Active Challenge Achievements"));

    const float y_advance = image_size.y + spacing;
    const float acheivement_name_offset = (image_size.y - font->LegacySize) / 2.0f;
    ...
    for (const AchievementChallengeIndicator& indicator : s_active_challenge_indicators)
    {
      ...
      const ImVec2 text_width(font->CalcTextSizeA(font->LegacySize, max_non_ellipised_text_width, 0.0f, achievement_title,
      ...
      {
        dl->AddText(font, font->LegacySize, text_position, text_color, achievement_title, remaining_text, 0.0f,
                    &text_bbox);
        dl->AddText(font, font->LegacySize, ImVec2(text_position.x + text_width.x, text_position.y), text_color, "...",
                    nullptr, 0.0f, &text_bbox);
      }
      else
      {
        dl->AddText(font, font->LegacySize, text_position, text_color, achievement_title, achievement_title_end, 0.0f,
                    &text_bbox);
      }
...
void Achievements::DrawAchievementsWindow()
{
        ...
        ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse))
  {
    ...
    if (visible)
    {
      ...
      const ImRect title_bb(ImVec2(left, top), ImVec2(right, top + g_large_font->LegacySize));
      text.assign(s_game_title);

      if (s_hardcore_mode)
        text.append(TRANSLATE_SV("Achievements", " (Hardcore Mode)"));

      top += g_large_font->LegacySize + spacing;
      ...
      const ImRect summary_bb(ImVec2(left, top), ImVec2(right, top + g_medium_font->LegacySize));
      if (s_game_summary.num_core_achievements > 0)
      {
      ...
      top += g_medium_font->LegacySize + spacing;
      ...
      if (s_game_summary.num_core_achievements > 0)
      {
        ...
        dl->AddText(g_medium_font, g_medium_font->LegacySize, text_pos,
...
void Achievements::DrawAchievement(const rc_client_achievement_t* cheevo)
{
  ...
  const ImVec2 points_template_size(
    g_medium_font->CalcTextSizeA(g_medium_font->LegacySize, FLT_MAX, 0.0f, TRANSLATE("Achievements", "XXX points")));
  ...
  const ImVec2 summary_text_size(g_medium_font->CalcTextSizeA(
    g_medium_font->LegacySize, FLT_MAX, summary_wrap_width, cheevo->description, cheevo->description + summary_length));

  // Messy, but need to undo LayoutScale in MenuButtonFrame()...
  const float extra_summary_height = LayoutUnscale(std::max(summary_text_size.y - g_medium_font->LegacySize, 0.0f));
  ...
  const float midpoint = bb.Min.y + g_large_font->LegacySize + spacing;
  text = TRANSLATE_PLURAL_SSTR("Achievements", "%n points", "Achievement points", cheevo->points);
  const ImVec2 points_size(
    g_medium_font->CalcTextSizeA(g_medium_font->LegacySize, FLT_MAX, 0.0f, text.c_str(), text.end_ptr()));
  ...
  const ImVec2 right_icon_size(g_large_font->CalcTextSizeA(g_large_font->LegacySize, FLT_MAX, 0.0f, right_icon_text));
  ...
  const ImRect summary_bb(ImVec2(text_start_x, midpoint),
                          ImVec2(points_start, midpoint + g_medium_font->LegacySize + extra_summary_height));
  ...
  else if (is_measured)
  {
    ...
    const float top = midpoint + g_medium_font->LegacySize + progress_spacing;
...
void Achievements::DrawLeaderboardsWindow()
{
  ...
  const float rank_column_width =
    g_large_font->CalcTextSizeA(g_large_font->LegacySize, std::numeric_limits<float>::max(), -1.0f, "99999").x;
  const float name_column_width =
    g_large_font
      ->CalcTextSizeA(g_large_font->LegacySize, std::numeric_limits<float>::max(), -1.0f, "WWWWWWWWWWWWWWWWWWWWWW")
      .x;
  const float time_column_width =
    g_large_font->CalcTextSizeA(g_large_font->LegacySize, std::numeric_limits<float>::max(), -1.0f, "WWWWWWWWWWW").x;
        ...
        ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse))
  {
    ...
    if (visible)
    {
      ...
      if (is_leaderboard_open)
      {
        const ImRect subtitle_bb(ImVec2(left, top), ImVec2(right, top + g_large_font->LegacySize));
        text.assign(s_open_leaderboard->title);

        top += g_large_font->LegacySize + spacing_small;
      ...
      const ImRect summary_bb(ImVec2(left, top), ImVec2(right, top + g_medium_font->LegacySize));
      top += g_medium_font->LegacySize + spacing_small;
      ...
      {
        const ImRect hardcore_warning_bb(ImVec2(left, top), ImVec2(right, top + g_medium_font->LegacySize));
        top += g_medium_font->LegacySize + spacing_small;
      ...
      if (is_leaderboard_open)
      {
        ...
        const ImVec2 line_start(bb.Min.x, bb.Min.y + g_large_font->LegacySize + line_padding);
...
{
  RA_InitClient((HWND)main_window_handle, "OpenSeason", g_scm_tag_str);
