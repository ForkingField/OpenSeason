...
bool CubebAudioStream::Initialize(const char* driver_name, const char* device_name, Error* error)
{
  ...
  int rv =
    cubeb_init(&m_context, "OpenSeason", g_settings.audio_driver.empty() ? nullptr : g_settings.audio_driver.c_str());
  if (rv != CUBEB_OK)
  {
...
std::vector<AudioStream::DeviceInfo> AudioStream::GetCubebOutputDevices(const char* driver, u32 sample_rate)
{
  ...
  cubeb* context;
  int rv = cubeb_init(&context, "OpenSeason", (driver && *driver) ? driver : nullptr);
  if (rv != CUBEB_OK)
  {
