...
{
  return QStringLiteral("OpenSeason %1").arg(QLatin1StringView(g_scm_tag_str));
}
...
bool QtHost::InitializeConfig(std::string settings_filename)
{
      ...
      settings_version != SETTINGS_VERSION)
  {
    ...
    {
      QMessageBox::critical(
        nullptr, QStringLiteral("OpenSeason"),
        QStringLiteral(
          "Failed to save configuration to\n\n%1\n\nThe error was: %2\n\nPlease ensure this directory is writable. You "
          "can also try portable mode by creating portable.txt in the same directory you installed OpenSeason into.")
...
bool QtHost::SetCriticalFolders()
{
  ...
  const std::string rcc_path = Path::Combine(EmuFolders::Resources, "openseason-qt.rcc");
...
bool QtHost::SetDataDirectory()
{
...
#if defined(_WIN32)
  // On Windows, use My Documents\OpenSeason.
  PWSTR documents_directory;
  if (SUCCEEDED(SHGetKnownFolderPath(FOLDERID_Documents, 0, NULL, &documents_directory)))
  {
    if (std::wcslen(documents_directory) > 0)
      EmuFolders::DataRoot = Path::Combine(StringUtil::WideStringToUTF8String(documents_directory), "OpenSeason");
  ...
  {
    // Use ~/.local/share/openseason otherwise.
    ...
    if (home_dir)
    {
      ...
      EmuFolders::DataRoot = Path::RealPath(Path::Combine(share_dir, "openseason"));
    }
  }
#elif defined(__APPLE__)
  static constexpr char MAC_DATA_DIR[] = "Library/Application Support/OpenSeason";
  const char* home_dir = getenv("HOME");
  if (home_dir)
    ...
    if (!FileSystem::EnsureDirectoryExists(EmuFolders::DataRoot.c_str(), false, &error))
    {
      ...
      QMessageBox::critical(
        ...
        QStringLiteral("Failed to create data directory at path\n\n%1\n\nThe error was: %2\nPlease ensure this "
                       ...
                       "directory you installed OpenSeason into.")
...
void QtHost::PrintCommandLineVersion()
{
  ...
  std::fprintf(stderr, "OpenSeason %s (%s)\n", g_scm_tag_str, g_scm_branch_str);
  std::fprintf(stderr, "https://github.com/ForkingField/OpenSeason\n");
  std::fprintf(stderr, "\n");
}
