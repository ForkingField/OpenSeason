  ...
  : QWidget(parent), m_dialog(dialog)
{
  ...
  dialog->registerWidgetHelp(m_ui.enable, tr("Enable Achievements"), tr("Unchecked"),
                             tr("When enabled and logged in, OpenSeason will scan for achievements on startup."));
  ...
  dialog->registerWidgetHelp(m_ui.spectatorMode, tr("Enable Spectator Mode"), tr("Unchecked"),
                             tr("When enabled, OpenSeason will assume all achievements are locked and not send any "
                                "unlock notifications to the server."));
  dialog->registerWidgetHelp(
    m_ui.unofficialAchievements, tr("Test Unofficial Achievements"), tr("Unchecked"),
    tr("When enabled, OpenSeason will list achievements from unofficial sets. Please note that these achievements are "
...
void AchievementSettingsWidget::updateLoginState()
{
  ...
  if (logged_in)
  {
    ...
    const QDateTime login_timestamp(QDateTime::fromSecsSinceEpoch(static_cast<qint64>(login_unix_timestamp), QTimeZone::utc()));
