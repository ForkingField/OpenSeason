...
if [ "$#" -ne 4 ]; then
    echo "Syntax: $0 <path to openseason directory> <path to build directory> <deps prefix> <output name>"
    exit 1
fi
...
BINARY=openseason-qt
APPDIRNAME=OpenSeason.AppDir
STRIP=strip
...
if [ ! -f "$LINUXDEPLOY" ]; then
	retry_command wget -O "$LINUXDEPLOY" https://github.com/stenzek/openseason-ext-qt-minimal/releases/download/linux/linuxdeploy-x86_64.AppImage
	chmod +x "$LINUXDEPLOY"
fi

if [ ! -f "$LINUXDEPLOY_PLUGIN_QT" ]; then
	retry_command wget -O "$LINUXDEPLOY_PLUGIN_QT" https://github.com/stenzek/openseason-ext-qt-minimal/releases/download/linux/linuxdeploy-plugin-qt-x86_64.AppImage
...
NO_STRIP="1" \
$LINUXDEPLOY --plugin qt --appdir="$OUTDIR" --executable="$BUILDDIR/bin/openseason-qt" ${EXTRA_LIBS_ARGS[@]} \
--desktop-file="$ROOTDIR/scripts/io.github.ForkingField.OpenSeason.desktop" \
--icon-file="$ROOTDIR/scripts/io.github.ForkingField.OpenSeason.png" \
